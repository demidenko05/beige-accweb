...
<div class="card cat-name">
  ...
  <div class="card-body">
    <c:forEach var="ord" items="${rvs.orders}">
        ...
        <table class="table table-sm">
          <thead>
            <tr>
              ...
              <c:if test="${ord.toTx.unscaledValue() gt 0 && rvs.tsrg.txExcl && !rvs.txRules.stIb}">
      ...
      <c:if test="${ord.toTx.unscaledValue() gt 0}">
...
<c:if test="${ifPayPal}">
  ...
  <script>
    paypal.Button.render({
      env: '${rvs.pmde}', // 'sandbox' or 'production'
      // Set up the payment:
      // 1. Add a payment callback
      payment: function(data, actions) {
        // 2. Make a request to your server
        return actions.request.post('?prc=PrPpl&prcRed=${prcRed}&catlId=${catlId}')
          .then(function(res) {
