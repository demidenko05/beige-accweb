...
<c:if test="${now.time - rvs.buyr.lsTm lt 1800000}">
  <c:set var="isUnSign" value="${false}" scope="request"/>
</c:if>
<c:if test="${now.time - rvs.buyr.lsTm gt 1800000}">
    ...
    <nav class="navbar navbar-expand-sm navbar-dark bg-dark fixed-top">
      ...
      <div id="navbar1" class="collapse navbar-collapse">
              ...
              <c:if test="${cat1l.subcatls.size() gt 0}">
                <li class="nav-item dropdown">
                  ...
                  <div class="dropdown-menu" aria-labelledby="dropdown${status.index}">
                    <a class="dropdown-item" href="${urlPrf}?prc=WsPg&catlId=${cat1l.catl.iid}">${i18n.getMsg("All", rvs.upf.lng.iid)}</a>
                    <c:forEach var="cat2l" items="${cat1l.subcatls}">
                      <c:if test="${cat2l.subcatls.size() gt 0}">
                        <a class="dropdown-item" href="#" data-toggle="modal" data-target="#subcatalogsMdl${cat2l.catl.iid}">${utlTrJsp.catlToStr(cat2l.catl, rvs.i18Cats, rvs.upf.lng.iid)}</a>
                      </c:if>
                      <c:if test="${cat2l.subcatls.size() eq 0}">
                        <a class="dropdown-item" href="${urlPrf}?prc=WsPg&catlId=${cat2l.catl.iid}">${utlTrJsp.catlToStr(cat2l.catl, rvs.i18Cats, rvs.upf.lng.iid)}</a>
                      </c:if>
                    </c:forEach>
                  </div>
                </li>
        ...
        <ul class="navbar-nav navbar-right">
          ...
          <li class="nav-item"><a href="#" class="nav-link" data-toggle="tooltip" title="${i18n.getMsg('About', rvs.upf.lng.iid)}" onclick="$('#infoDlg').modal('toggle');"><span class="oi oi-info" aria-hidden="true"></span></a></li>
