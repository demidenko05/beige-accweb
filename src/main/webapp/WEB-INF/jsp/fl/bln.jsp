...
<div class="input-line">
  ...
  <c:if test="${hldUvd.fltMp[frcd].contains(fdNm)}">
    ...
    ${i18n.getMsg(fdNm, rvs.upf.lng.iid)}: ${i18n.getMsg(hldUvd.fltMp[parVal], rvs.upf.lng.iid)}
  ...
  <c:if test="${!hldUvd.fltMp[frcd].contains(fdNm)}">
    ...
    <select name="${parVal}" onchange="bsInpChn(this);">
      ...
      <c:set var="selDis" value=""/>
      <c:if test="${hldUvd.fltMp[parVal].toString() eq 'true'}"> <c:set var="selDis" value="selected"/> </c:if>
      <option value="true" ${selDis}>${i18n.getMsg("Yes", rvs.upf.lng.iid)}</option>
      <c:set var="selDis" value=""/>
      <c:if test="${hldUvd.fltMp[parVal].toString() eq 'false'}"> <c:set var="selDis" value="selected"/> </c:if>
