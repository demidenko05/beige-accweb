  ...
  <c:if test="${rvs.ords != null}">
    ...
    <c:forEach var="ord" items="${rvs.ords}" varStatus="status">
      ...
      ${divCnt}
        ...
        ${i18n.getMsg(ord.stat, rvs.upf.lng.iid)}, ${i18n.getMsg("tot", rvs.upf.lng.iid)} ${ord.tot}${ord.curr.stCo}
        ...
        <c:if test="${not empty ord.place}">
          , ${i18n.getMsg("place", rvs.upf.lng.iid)}: ${ord.place.nme}
        </c:if></h6>
    ...
    <c:if test="${rvs.tstg.colCnt != 1 && ords.size() mod 2 != 0}">
      </div>
    </c:if>
  ...
  <c:if test="${rvs.sords != null}">
    ...
    <c:forEach var="ord" items="${rvs.sords}" varStatus="status">
      ...
      ${divCnt}
        ...
        ${i18n.getMsg("seller", rvs.upf.lng.iid)}: ${ord.sel.seller.nme}, ${i18n.getMsg(ord.stat, rvs.upf.lng.iid)}, ${i18n.getMsg("tot", rvs.upf.lng.iid)} ${ord.tot}${ord.curr.stCo}
        <c:if test="${ord.toTx.doubleValue() gt 0 && rvs.tstg.txExcl}">(${i18n.getMsg("txExcl", rvs.upf.lng.iid)}),</c:if>
        <c:if test="${ord.toTx.doubleValue() gt 0 && !rvs.tstg.txExcl}">(${i18n.getMsg("txIncl", rvs.upf.lng.iid)}),</c:if>
        ${i18n.getMsg("toTx", rvs.upf.lng.iid)} ${ord.toTx}${ord.curr.nme},
        ${i18n.getMsg("paym", rvs.upf.lng.iid)}: ${i18n.getMsg(ord.paym, rvs.upf.lng.iid)},
